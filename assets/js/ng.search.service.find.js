...
;                                searchData['term:'+ filter.prefix + filter.filter.param] = filter.filter.value.replace(/\{val\}/g, search_value.join(','));
            ...
            function apiFind(entity, searchData, page, action) {
               ...
               // delete searchData['@count'];
				...
				if (entity === 'event'){ //ordena eventos
                     searchData['@order'] = 'createTimestamp DESC, name ASC';
					// searchData['@order'] = 'createTimestamp DESC, name ASC';
				}
				if (entity === 'space'){  //ordena espaços
				     searchData['@order'] = 'name ASC';
				}
                if (entity === 'agent'){  //ordena agentes
				     searchData['@order'] = 'name ASC';
				}
			    if (entity === 'project'){  //ordena projetos	
				 	searchData['@order'] = 'isVerified DESC, registrationFrom ASC registrationTo ASC';
				}
				delete searchData['@count'];
